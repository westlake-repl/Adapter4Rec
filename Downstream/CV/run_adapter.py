...
from transformers import ViTMAEModel, ViTForImageClassification
...
from model import Model, ModelCPC, SoftPrompt, VITAdaptedSelfOutput, SASRecAdaptedSelfOutput, \
    VITAdaptedParallelSelfOutput, SASRecParallelAdaptedSelfOutput, \
    VITPfeifferAdaptedSelfOutput, \
    VITKAdaptedCVModel, SASRecKAdaptedTransformerBlocks, VITAdaptedOutput, VITAdaptedParallelOutput, \
    SASRecPfeifferV2AdaptedSelfOutput, VITCompacterAdaptedSelfOutput, VITCompacterAdaptedOutput, \
    SASRecCompacterAdaptedSelfOutput, PHMLinear
...
def add_kadapter_adapter_to_vit(cv_model, args):
    return VITKAdaptedCVModel(cv_model, args)


def add_pfeiffer_v2_adapter_to_sasrec(transformer_block, args):
    return SASRecPfeifferV2AdaptedSelfOutput(transformer_block, args)
...
def add_pfeiffer_adapter_to_vit(self_output, args):
    return VITPfeifferAdaptedSelfOutput(self_output, args)
...
def add_parallel_adapter_to_vit_selfoutput(self_output, args):
    return VITAdaptedParallelSelfOutput(self_output, args)


def add_parallel_adapter_to_vit_output(self_output, args):
    return VITAdaptedParallelOutput(self_output, args)
...
# add compacters
def add_compacter_to_sasrec(transformer_block, args):
    return SASRecCompacterAdaptedSelfOutput(transformer_block, args)


def add_compacter_to_vit_selfoutput(self_output, args):
    return VITCompacterAdaptedSelfOutput(self_output, args)


def add_compacter_to_vit_output(self_output, args):
    return VITCompacterAdaptedOutput(self_output, args)
    ...
    if 'None' not in args.load_ckpt_name:
        if "pfeiffer_ver2" in args.adapter_type:
            ...
            for index, layer_module in enumerate(
                    model.cv_encoder.image_net.vit.encoder.layer):
                layer_module.attention.output = add_adapter_to_vit_selfoutput(layer_module.attention.output, args).to(
                    local_rank)
            ...
            for index, transformer_block in enumerate(
                ...
                model.user_encoder.transformer_encoder.transformer_blocks[index] = add_pfeiffer_v2_adapter_to_sasrec(
                    transformer_block, args).to(local_rank)
        elif 'kadapter' in args.adapter_type:
            model.cv_encoder.image_net.vit.encoder = add_kadapter_adapter_to_vit(
                model.cv_encoder.image_net.vit.encoder, args).to(local_rank)
            ...
            for index, layer_module in enumerate(
                    model.cv_encoder.image_net.vit.encoder.layer):
                layer_module.attention.attention.query = lora.Linear(768, 768, r=12).to(local_rank)
                layer_module.attention.attention.value = lora.Linear(768, 768, r=12).to(local_rank)
            ...
            for index, transformer_block in enumerate(
                ...
                model.user_encoder.transformer_encoder.transformer_blocks[index].multi_head_attention.w_Q = lora.Linear(
                    args.embedding_dim, args.embedding_dim, r=4).to(local_rank)
                model.user_encoder.transformer_encoder.transformer_blocks[index].multi_head_attention.w_V = lora.Linear(
                    args.embedding_dim, args.embedding_dim).to(local_rank)
            ...
            for index, layer_module in enumerate(
                    model.cv_encoder.image_net.vit.encoder.layer):
                layer_module.attention.output = add_compacter_to_vit_selfoutput(layer_module.attention.output,
                                                                                args).to(
                    local_rank)
                layer_module.output = add_compacter_to_vit_output(layer_module.output, args).to(local_rank)
            # adding adapters to the SASRec model
            for index, transformer_block in enumerate(
                    model.user_encoder.transformer_encoder.transformer_blocks):
                model.user_encoder.transformer_encoder.transformer_blocks[index] = add_compacter_to_sasrec(
                    transformer_block, args).to(local_rank)
            model = CompacterModel(args, model).to(local_rank)
        elif "prompt" in args.adapter_type:
            # adding soft prompt
            s_wte = SoftPrompt(cv_model.vit.embeddings,
                               n_tokens=args.n_tokens,
                               embed_dim=768)
            model.cv_encoder.image_net.vit.embeddings = s_wte.to(local_rank)
            for index, (name, param) in enumerate(model.named_parameters()):
                if "cv_encoder.image_net.classifier" in name:
            ...
            if "None" not in args.is_serial:
                ...
                if "mae" not in args.CV_model_load:
                    ...
                    for index, layer_module in enumerate(
                            model.cv_encoder.image_net.encoder.layer):
                        layer_module.attention.output = add_adapter_to_vit_selfoutput(layer_module.attention.output,
                                                                                      args).to(
                            local_rank)
                        layer_module.output = add_adapter_to_vit_output(layer_module.output, args).to(
                ...
                for index, layer_module in enumerate(
                        model.cv_encoder.image_net.vit.encoder.layer):
                    # layer_module.attention.output = add_parallel_adapter_to_vit_selfoutput(layer_module.attention.output,args).to(local_rank)
        ...
        Log_file.info(model_dir)
        Log_file.info(args.load_ckpt_name)
        ckpt_path2 = get_checkpoint(model_dir, args.load_ckpt_name)
        Log_file.info(f'{ckpt_path2}')
        ...
        torch.set_rng_state(checkpoint2['rng_state'])  # load torch random number generator state
        torch.cuda.set_rng_state(checkpoint2['cuda_rng_state'])  # load torch.cuda random number generator state
    else:
        print('no ckpt to load')
        assert 1 == 0, 'no ckpt to load, please check the ckpt name'
...
def train(args, use_modal, local_rank):
    ...
    if use_modal:
        ...
        if "compacter" in args.adapter_type:
            Log_file.info("***** {} parameters in images, {} parameters in model *****".format(
                len(list(model.module.model.cv_encoder.image_net.parameters())),
    ...
    for ep in range(args.epoch):
        ...
        if not need_break:
            ...
            run_eval_test(model, item_id_to_keys, users_history_for_test, users_test, args.batch_size, item_num,
                          use_modal,
                          args.mode, local_rank)
            model.train()
...
if __name__ == "__main__":
    ...
    if 'modal' in args.item_tower:
        ...
        dir_label = str(
            args.item_tower) + f'{args.arch}_{args.pretrained_recsys_model}_{model_load}_freeze_{args.freeze_paras_before}' + f"_{args.pretrained_recsys_model}" + f"_add_adapter_to_{args.adding_adapter_to}" + f"_adapter_cv_lr_{args.adapter_cv_lr}" + f"_adapter_sasrec_lr_{args.adapter_sasrec_lr}" + f"_adapter_down_size_{args.adapter_down_size}" + f"_cv_adapter_down_size_{args.cv_adapter_down_size}__serial_{args.is_serial}_layernorm_{args.finetune_layernorm}_{args.adapter_type}"
    else:
        is_use_modal = False
        model_load = 'id'
    ...
    model_dir = os.path.join('./checkpoint_' + dir_label, 'cpt_' + log_paras + args.pretrained_recsys_model)
